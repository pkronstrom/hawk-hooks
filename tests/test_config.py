...
class TestProfiles:
    def test_save_and_load(self, v2_env):
        ...
        config.save_profile("web-fullstack", data)
        loaded = config.load_profile("web-fullstack")
        assert loaded is not None
        assert loaded["skills"] == ["tdd", "react-patterns"]

    def test_load_missing_returns_none(self, v2_env):
        assert config.load_profile("nonexistent") is None

    def test_list_profiles(self, v2_env):
        config.save_profile("web", {"name": "web"})
        config.save_profile("api", {"name": "api"})
    ...
    def test_profile_name_path_traversal_rejected(self, v2_env):
        with pytest.raises(ValueError):
            config.load_profile("../../../etc/passwd")
        with pytest.raises(ValueError):
            config.save_profile("../../../etc/passwd", {"name": "bad"})
...
class TestDirConfig:
    def test_save_and_load(self, v2_env, tmp_path):
        ...
        config.save_dir_config(project, data)
        loaded = config.load_dir_config(project)
        assert loaded is not None
        assert loaded["profile"] == "web-fullstack"

    def test_load_missing_returns_none(self, tmp_path):
        assert config.load_dir_config(tmp_path / "nonexistent") is None
...
class TestDirectoryIndex:
    ...
    def test_unregister(self, v2_env, tmp_path):
        ...
        config.unregister_directory(project)
        dirs = config.get_registered_directories()
...
class TestToolGlobalDir:
    def test_default_dirs(self, v2_env):
        path = config.get_tool_global_dir(Tool.CLAUDE)
...
class TestConfigChain:
    def test_empty_chain_when_no_dirs_registered(self, v2_env, tmp_path):
        ...
        project.mkdir()
        chain = config.get_config_chain(project)
        assert chain == []

    def test_single_dir_in_chain(self, v2_env, tmp_path):
        project = tmp_path / "project"
        project.mkdir()
        config.save_dir_config(project, {"skills": {"enabled": ["tdd"]}})
        config.register_directory(project)
    ...
    def test_nested_dirs_outermost_first(self, v2_env, tmp_path):
        ...
        config.save_dir_config(root, {"profile": "fullstack"})
        config.register_directory(root)
        config.save_dir_config(child, {"skills": {"enabled": ["react"]}})
        config.register_directory(child)

        chain = config.get_config_chain(child)
        assert len(chain) == 2
    ...
    def test_unrelated_dirs_excluded(self, v2_env, tmp_path):
        ...
        config.save_dir_config(project_a, {"skills": {"enabled": ["a"]}})
        config.register_directory(project_a)
        config.save_dir_config(project_b, {"skills": {"enabled": ["b"]}})
        config.register_directory(project_b)

        chain = config.get_config_chain(project_a)
        assert len(chain) == 1
    ...
    def test_nearest_registered_directory_for_nested_subdir(self, v2_env, tmp_path):
        ...
        nearest = config.get_nearest_registered_directory(child)
    ...
    def test_nearest_registered_directory_prefers_innermost(self, v2_env, tmp_path):
        ...
        config.save_dir_config(root, {"skills": {"enabled": ["base"]}})
        config.register_directory(root)
        config.save_dir_config(child, {"skills": {"enabled": ["ui"]}})
        config.register_directory(child)

        nearest = config.get_nearest_registered_directory(leaf)
...
class TestAutoRegister:
    def test_auto_registers_when_config_exists(self, v2_env, tmp_path):
        ...
        config.auto_register_if_needed(project)
        dirs = config.get_registered_directories()
        assert str(project.resolve()) in dirs
...
class TestPruneStale:
    def test_removes_stale_entries(self, v2_env, tmp_path):
        ...
        pruned = config.prune_stale_directories()
...
class TestPackages:
    ...
    def test_get_package_for_item_found(self, v2_env):
        ...
        assert config.get_package_for_item("skill", "tdd") == "pkg-a"
    ...
    def test_get_package_for_item_not_found(self, v2_env):
        ...
        assert config.get_package_for_item("skill", "other") is None
    ...
    def test_record_package_updates_existing(self, v2_env):
        config.record_package("pkg", "url1", "c1", [])
        config.record_package("pkg", "url2", "c2", [{"type": "skill", "name": "new", "hash": "h"}])
        loaded = config.load_packages()
...
class TestContentHashing:
    def test_hash_file(self, tmp_path):
        ...
        result = config.hash_registry_item(f)
        assert len(result) == 8
    ...
    def test_hash_file_deterministic(self, tmp_path):
        f = tmp_path / "test.md"
        f.write_text("hello world")
        assert config.hash_registry_item(f) == config.hash_registry_item(f)

    def test_hash_file_changes_with_content(self, tmp_path):
        f = tmp_path / "test.md"
        f.write_text("version 1")
        h1 = config.hash_registry_item(f)
        f.write_text("version 2")
        h2 = config.hash_registry_item(f)
        assert h1 != h2
    ...
    def test_hash_dir(self, tmp_path):
        ...
        result = config.hash_registry_item(d)
        assert len(result) == 8
    ...
    def test_hash_dir_deterministic(self, tmp_path):
        ...
        (d / "a.md").write_text("a")
        (d / "b.md").write_text("b")
        assert config.hash_registry_item(d) == config.hash_registry_item(d)

    def test_hash_dir_changes_with_content(self, tmp_path):
        d = tmp_path / "skill"
        d.mkdir()
        (d / "a.md").write_text("v1")
        h1 = config.hash_registry_item(d)
        (d / "a.md").write_text("v2")
        h2 = config.hash_registry_item(d)
        assert h1 != h2

    def test_hash_nonexistent(self, tmp_path):
        result = config.hash_registry_item(tmp_path / "nope")
