...
Redesign hawk-hooks to use auto-discovered hook scripts and generated bash runners for better extensibility and performance.

## Goals

1. Drop-in scripts: add a script, run `hawk-hooks toggle`, it appears
...
4. Simple deps: Python venv managed by hawk-hooks, others documented
...
```bash
#!/usr/bin/env bash
# Auto-generated by hawk-hooks - do not edit
# Event: post_tool_use

set -euo pipefail

INPUT=$(cat)
HOOKS_DIR="$HOME/.config/hawk-hooks/hooks/post_tool_use"
VENV_PYTHON="$HOME/.config/hawk-hooks/.venv/bin/python"

# Check for project override
PROJECT_RUNNER=".claude/hawk-hooks/runners/post_tool_use.sh"
...
| `hawk-hooks install` | Register runners in Claude settings |
| `hawk-hooks uninstall` | Remove from Claude, offer cleanup |
| `hawk-hooks toggle` | Scan hooks + enable/disable + regenerate runners |
| `hawk-hooks status` | Show discovered hooks + enabled state |
| `hawk-hooks install-deps` | Install Python deps to venv, print others |

## First-Run Wizard

```
╭─────────────────────────────────────────╮
│  Welcome to hawk-hooks!               │
...
**Python:**
- Managed venv at `~/.config/hawk-hooks/.venv/`
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{"type": "command", "command": "~/.config/hawk-hooks/runners/pre_tool_use.sh"}]
    }, {
      "matcher": "Bash",
      "hooks": [{"type": "command", "command": "~/.config/hawk-hooks/runners/pre_tool_use.sh"}]
    }],
    "PostToolUse": [{
      "matcher": "Edit|Write|MultiEdit",
      "hooks": [{"type": "command", "command": "~/.config/hawk-hooks/runners/post_tool_use.sh"}]
    }],
    "Stop": [{
      "hooks": [{"type": "command", "command": "~/.config/hawk-hooks/runners/stop.sh"}]
    }]
  }
}
```
